...
def test_dec2dec():
    ...
    for dec in ["+14:21:45.003", "-99 04 22", "-00 01 23.456", "00 01"]:
        ...
        assert_approx_equal(ans, desired, err_msg=f"{ans} != {desired}")
...
def test_gcd():
    ...
    for ra1, dec1, ra2, dec2, dist in [
        (0, 0, 0, 1, 1),  # simple 1 deg offset
        (0, -90, 180, 90, 180),  # pole to pole
        (120, 89, 300, 89, 2.0),  # over the pole
        ...
        assert_almost_equal(
            ans,
            dist,
            err_msg=f"{ra1:5.2f},{dec1:5.2f} <-> {ra2:5.2f},{dec2:5.2f} == {dist:g} != {ans:g}",
        )


def test_bear():
    """Test bearing calculation"""
    for ra1, dec1, ra2, dec2, bear in [
        (0, 0, 0, 1, 0),
        (0, 0, 180, 90, 0),
        (0, 0, 179.99999, 0, 90),
        (0, 0, 180.00001, 0, -90),
        ...
        assert_almost_equal(
            ans,
            bear,
            err_msg=f"{ra1:5.2f},{dec1:5.2f} <-> {ra2:5.2f},{dec2:5.2f} == {bear:g} != {ans:g}",
        )


def test_translate():
    """Test the translate function"""
    for (ra1, dec1), (r, theta), (ra2, dec2) in [
        ((0, 0), (1, 0), (0, 1)),
        # over the pole
        ((45, 89.75), (0.5, 0), (225, 89.75)),
        # negative r
        ((12, -45), (-1, 180), (12, -44)),
    ]:
        ...
        assert_almost_equal(
            ans,
            (ra2, dec2),
            err_msg="{:5.2f},{:5.2f} -> {:g},{:g} -> {:5.2f},{:5.2f} != {:g},{:g}".format(
                ra1, dec1, r, theta, ra2, dec2, *ans
            ),
        )
...
def test_dist_rhumb():
    ...
    for ra1, dec1, ra2, dec2, dist in [(0, 0, 0, 1, 1), (0, 0, 180, 0, 180)]:
...
def test_bear_rhumb():
    ...
    for ra1, dec1, ra2, dec2, bear in [(0, 0, 0, 1, 0), (0, 0, 180, 0, 90)]:
