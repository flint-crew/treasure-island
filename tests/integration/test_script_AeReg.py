...
def test_noregroup():
    # should run
    AeReg.main(["--input", catfile, "--table", tempfile + ".csv", "--noregroup"])
    os.remove(tempfile + "_comp.csv")


def test_regroup():
    AeReg.main(
        ["--input", catfile, "--table", tempfile + ".csv", "--debug", "--eps", "1"]
    )
    os.remove(tempfile + "_comp.csv")

    AeReg.main(["--input", catfile, "--table", tempfile + ".csv", "--ratio", "1.2"])
    os.remove(tempfile + "_comp.csv")
...
def test_broken_catalogue():
    ...
    AeReg.main(["--input", shortcat, "--table", tempfile, "--debug"])
