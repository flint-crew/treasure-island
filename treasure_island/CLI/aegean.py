...
def check_projection(filename, options, log):
    ...
    if "SIN" not in header["CTYPE1"] and options.imgpsf is None:
        ...
        log.warning(
            f"For projection {projection} you should consider supplying a psf via --psf"
        )
...
def main(argv=()):
    ...
    if options.nopositive and not options.negative:
        log.warning(
            "Requested no positive sources, but no negative sources. Nothing to find."
        )
        return 0
    ...
    if options.beam is not None:
        ...
        log.info(
            f"Beam is {options.beam.a} deg x {options.beam.b} deg with pa {options.beam.pa}"
        )
