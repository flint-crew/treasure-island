...
from treasure_island.fits_tools import compress
...
def sigmaclip(arr, lo, hi, reps=10):
    ...
    for count in range(int(reps)):  # noqa: B007
        ...
        logging.debug(f"No stopping criteria was reached after {count} cycles")
...
def sigma_filter(filename, region, step_size, box_size, shape, domask, cube_index):
    ...
    with fits.open(filename, memmap=True, do_not_scale_image_data=True) as a:
        if NAXIS == 2:
            ...
            data = np.squeeze(
                a[0].section[cube_index, data_row_min:data_row_max, 0 : shape[1]]
            )
        elif NAXIS == 4:
            data = np.squeeze(
    ...
    data[0 + ymin - data_row_min : data.shape[0] - (data_row_max - ymax), :] -= ibkg[
        ymin:ymax, :
    ]
    logging.debug(".. done ")
    ...
    if domask:
        ...
        mask = ~np.isfinite(
            data[0 + ymin - data_row_min : data.shape[0] - (data_row_max - ymax), :]
        )
...
def filter_mc_sharemem(
    filename, step_size, box_size, cores, shape, nslice=None, domask=True, cube_index=0
):
    """
...
def filter_image(
    ...
    step_size=None,
    box_size=None,
    twopass=False,  # Deprecated  # noqa: ARG001
    cores=None,
    mask=True,
    compressed=False,
    nslice=None,
    ...
    if compressed and step_size[0] != step_size[1]:
        ...
        logging.info(f"Changing grid to be {step_size} so we can compress the output")
    ...
    if "BMAJ" in header and "BMIN" in header:
        ...
        if "CDELT1" in header:
            ...
            if (
                "CD1_2" in header
                and "CD2_1" in header
                and (header["CD1_2"] != 0 or header["CD2_1"] != 0)
            ):
                logging.warning(
