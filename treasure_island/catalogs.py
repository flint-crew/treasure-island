...
from treasure_island.models import ComponentSource, classify_catalog
...
def check_table_formats(files):
    ...
    for t in files.split(","):
        ...
        if ext not in formats:
            cont = False
            log.warning(f"Format not supported for {t} ({ext})")
    if not cont:
...
def save_catalog(filename, catalog, meta=None, prefix=None):
    ...
    if extension in ["ann", "reg"]:
    ...
    elif extension in ascii_table_formats:
        write_catalog(
            filename,
            catalog,
        ...
        log.warning(f"extension not recognised {extension}")
...
def load_catalog(filename):
    ...
    if fmt in ["csv", "tab", "tex"] and fmt in supported:
        ...
        catalog = list(zip(t.columns["ra"], t.columns["dec"], strict=False))

    elif fmt in ["vo", "vot", "xml"] and fmt in supported:
        log.info(f"Reading file {filename}")
        t = parse_single_table(filename)
        catalog = list(
            zip(t.array["ra"].tolist(), t.array["dec"].tolist(), strict=False)
        )
        ...
        lines = [
            a.strip().split()
            for a in open(filename).readlines()
            if not a.startswith("#")
        ]
        try:
            ...
            log.error(f"Catalog file {filename} not loaded")
            msg = "Could not determine file format"
            raise Exception(msg)
...
def load_table(filename):
    ...
    if fmt in ["csv", "tab", "tex"] and fmt in supported:
        ...
        log.error(f"{filename} [{fmt}]")
        msg = "Table format not recognized or supported"
        raise Exception(msg)
    return t
...
def write_table(table, filename):
    ...
    try:
        ...
        msg = f"Cannot auto-determine format for {fmt}"
        raise Exception(msg)
...
def write_catalog(filename, catalog, fmt=None, meta=None, prefix=None):
    ...
    pre = "" if prefix is None else prefix + "_"
...
def writeFITSTable(filename, table):
    ...
    def FITSTableType(val):
        ...
        if isinstance(val, bool):
            ...
            log.warning(f"Column {val} is of unknown type {type(val)}")
    ...
    for name in table.colnames:
        ...
        if name.startswith("err_"):
            ...
            fmt = f"{max(len(val) for val in table[name])}A"
        else:
...
def writeIslandContours(filename, catalog, fmt="reg"):
    ...
    for c in catalog:
        contour = c.contour
        if len(contour) > 1:
            for p1, p2 in itertools.pairwise(contour):
                print(
...
def writeIslandBoxes(filename, catalog, fmt):
    ...
    print(f"#Aegean version {__version__}-({__date__})", file=out)
...
def writeAnn(filename, catalog, fmt):
    ...
    if len(cat) > 0:
        ...
        if fmt == "ann":
            new_file = re.sub(".ann$", f"_{suffix}.ann", filename)
            out = open(new_file, "w")
            print(f"#Aegean version {__version__}-({__date__})", file=out)
            ...
            new_file = re.sub(".reg$", f"_{suffix}.reg", filename)
            out = open(new_file, "w")
            print(f"#Aegean version {__version__}-({__date__})", file=out)
            print("fk5", file=out)
            formatter = (
        ...
        for ra, dec, bmaj, bmin, pa, name in zip(
            ras, decs, bmajs, bmins, pas, names, strict=False
        ):
    ...
    if len(islands) > 0:
        if fmt == "reg":
            ...
            log.warning(f"format {fmt} not supported for island annotations")
            return
...
def writeDB(filename, catalog, meta=None):
    ...
    def sqlTypes(obj, names):
        ...
        for n in names:
            ...
            # float32 is bugged and claims not to be a float
                ...
                log.warning(f"Column {n} is of unknown type {type(n)}")
                log.warning("Using VARCHAR")
                types.append("VARCHAR")
        return types

    if os.path.exists(filename):
        log.warning(f"overwriting {filename}")
        os.remove(filename)
    conn = sqlite3.connect(filename)
    db = conn.cursor()
    # determine the column names by inspecting the catalog class
    for t, tn in zip(
        classify_catalog(catalog), ["components", "islands", "simples"], strict=False
    ):
        if len(t) < 1:
        ...
        stmnt = ",".join(
            [f"{a} {b}" for a, b in zip(col_names, col_types, strict=False)]
        )
        db.execute(f"CREATE TABLE {tn} ({stmnt})")
        stmnt = "INSERT INTO {} ({}) VALUES ({})".format(
            tn, ",".join(col_names), ",".join(["?" for i in col_names])
        )
        # expend the iterators that are created by python 3+
        data = list(map(nulls, [r.as_list() for r in t]))
        db.executemany(stmnt, data)
        log.info(f"Created table {tn}")
    ...
    conn.close()
    log.info(f"Wrote file {filename}")
